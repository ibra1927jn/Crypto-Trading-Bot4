...
class TestCalculateAll:
    def test_adds_rsi_column(self, indicators) -> None:
        df = make_ohlcv(100)
        result = indicators.calculate_all(df)
        assert 'rsi' in result.columns

    def test_adds_macd_columns(self, indicators) -> None:
        df = make_ohlcv(100)
        result = indicators.calculate_all(df)
        macd_cols = [c for c in result.columns if 'MACD' in c]
        assert len(macd_cols) >= 2

    def test_adds_bollinger_columns(self, indicators) -> None:
        df = make_ohlcv(100)
        result = indicators.calculate_all(df)
        bb_cols = [c for c in result.columns if c.startswith('BB')]
        assert len(bb_cols) >= 3

    def test_none_input(self, indicators) -> None:
        result = indicators.calculate_all(None)
        assert result is None

    def test_empty_input(self, indicators) -> None:
...
class TestMACDSignal:
    def test_returns_tuple(self, indicators, df_with_indicators) -> None:
        result = indicators.get_macd_signal(df_with_indicators)
        assert isinstance(result, tuple)
        assert len(result) == 2

    def test_signal_values(self, indicators, df_with_indicators) -> None:
    ...
    def test_crossover_buy(self, indicators) -> None:
    ...
    def test_crossover_sell(self, indicators) -> None:
...
class TestBollingerSignal:
    ...
    def test_buy_below_lower_band(self, indicators) -> None:
        df = make_ohlcv(200)
        df = indicators.calculate_all(df)
    ...
    def test_sell_above_upper_band(self, indicators) -> None:
        df = make_ohlcv(200)
        df = indicators.calculate_all(df)
...
class TestCombinedSignal:
    ...
    def test_oversold_rsi_gives_buy(self, indicators) -> None:
        df = make_ohlcv(200)
        df = indicators.calculate_all(df)
    ...
    def test_overbought_rsi_gives_sell(self, indicators) -> None:
        df = make_ohlcv(200)
        df = indicators.calculate_all(df)
...
class TestCalculateAllError:
    def test_calculate_all_with_bad_data(self, indicators) -> None:
    ...
    def test_macd_signal_empty_df(self, indicators) -> None:
        result = indicators.get_macd_signal(pd.DataFrame())
        assert result == ('NEUTRAL', 0.0)

    def test_bollinger_signal_empty_df(self, indicators) -> None:
        result = indicators.get_bollinger_signal(pd.DataFrame())
        assert result == ('NEUTRAL', 0.0)

    def test_combined_signal_empty_df(self, indicators) -> None:
        result = indicators.get_combined_signal(pd.DataFrame())
        assert result == ('NEUTRAL', 0.0)

    def test_indicators_summary_empty_df(self, indicators) -> None:
        result = indicators.get_indicators_summary(pd.DataFrame())
        assert result == {}


class TestMACDSignalError:
    def test_macd_signal_no_macd_columns(self, indicators) -> None:
...
class TestBollingerSignalError:
    def test_bollinger_signal_no_bb_columns(self, indicators) -> None:
    ...
    def test_bollinger_signal_no_close_column(self, indicators) -> None:
...
class TestCombinedSignalError:
    def test_combined_signal_no_rsi_column(self, indicators) -> None:
...
class TestIndicatorsSummaryError:
    def test_summary_no_rsi_column(self, indicators) -> None:
        """Error branch: df without 'rsi' column triggers exception handler."""
        df = pd.DataFrame({'close': [100, 101, 102]})
...
class TestGetIndicatorsSummary:
    def test_returns_dict_with_rsi(self, indicators, df_with_indicators) -> None:
        result = indicators.get_indicators_summary(df_with_indicators)
        assert isinstance(result, dict)
        assert 'rsi' in result

    def test_none_input_returns_empty(self, indicators) -> None:
        result = indicators.get_indicators_summary(None)
        assert result == {}

    def test_empty_input_returns_empty(self, indicators) -> None:
        result = indicators.get_indicators_summary(pd.DataFrame())
        assert result == {}


class TestBollingerSignalNeutral:
    def test_neutral_when_between_bands(self, indicators) -> None:
        df = make_ohlcv(200)
        df = indicators.calculate_all(df)
        bbl_col = next(c for c in df.columns if c.startswith('BBL'))
        bbu_col = next(c for c in df.columns if c.startswith('BBU'))
...
class TestCombinedSignalNeutral:
    def test_mid_rsi_neutral_bollinger(self, indicators) -> None:
...
class TestCombinedSignalBollingerContribution:
    def test_bollinger_buy_adds_to_signals(self, indicators) -> None:
    ...
    def test_bollinger_sell_adds_to_signals(self, indicators) -> None:
    ...
    def test_tied_signals_give_neutral(self, indicators) -> None:
    ...
    def test_precomputed_bollinger_is_reused(self, indicators, mocker) -> None:
